...
* Add new incubating API: `WebfunnyRumBuilder.setHttpSenderCustomizer()` to allow customization
  of the HTTP client used for sending data to Webfunny. This can be useful when devices are
  behind a proxy or API gateway (#742).
* Fix AndroidResource blending between webfunny version and upstream. (#757)
...
* Change session sampling strategy to be consistent with other webfunny RUM implementations (#698)
...
This is the first version of `webfunny-otel-android` that is based on the upstream version of
`opentelemetry-android`. Please note that this requires an additional project dependency,
as [documented here](https://docs.webfunny.com/observability/en/gdi/get-data-in/rum/android/install-rum-android.html#install-the-android-agent-as-a-dependency).
...
* Fix build to include `.aar` artifact. Please note that this version splits out an
  additional required dependency: `com.webfunny:opentelemetry-android-instrumentation`.
...
* `webfunny.rum.version` attribute has been renamed to `rum.sdk.version` (#524)
* new API: `OpenTelemetryRumBuilder.mergeResource()` to allow merging into resource instead of replacing (#524)
* new API: `webfunnyRumBuilder.disableBackgroundTaskReporting(applicationId)` - pass the `applicationId` to this
...
Version 1.0.0 marks the initial _stable_ release of `webfunny-otel-android`.
...
* Renamed the `webfunnyRumBuilder#disableNetworkMonitorEnabled()` method to
  `webfunnyRumBuilder#disableNetworkMonitor()` (#377)
...
* Introduced a `webfunnyRumBuilder` class and deprecated `Config` (#342)
...
* For okhttp, webfunnyRum now exposes a wrapper for your `OkHttpClient` which implements the `Call.Factory`
  interface. This `Call.Factory` will properly manage context propagation with asynchronous http calls.
* The okhttp Interceptor provided by webfunnyRum has been deprecated. Please use the `Call.Factory` from now on.
  The `createOkHttpRumInterceptor()` method will be removed in a future release.
* A new class (`com.webfunny.rum.StandardAttributes`) has been introduced to provide `AttributeKey`s for
...
* A new API on the `Config.Builder` allows redacting of spans or redacting/replacing Span attributes. See
  the new `filterSpans(Consumer<SpanFilterBuilder>)` method and the corresponding `com.webfunny.rum.SpanFilterBuilder` class
  for details.
...
* The `com.webfunny.rum.Config.Builder` class has been updated.
...
* New methods have been added to the `webfunnyRum` API to allow updating the "global" attributes that
...
* The values reported for network types have been updated to match OpenTelemetry semantic
  conventions.
* The webfunnyRum class has had a method added to return a no-op implementation of the webfunnyRum
  capabilities.
* The webfunnyRum initialization span now includes an attribute describing the features that have been
  configured.
