...
import com.webfunny.android.sample.databinding.FragmentFirstBinding;
...
public class FirstFragment extends Fragment {
    ...
    private WebfunnyRum webfunnyRum;

    @Override
    public View onCreateView(
            LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        webfunnyRum = WebfunnyRum.getInstance();
        okHttpClient = buildOkHttpClient(webfunnyRum);
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        binding.httpMe.setOnClickListener(
                v -> {
                    Span workflow = webfunnyRum.startWorkflow("User Login");
                    ...
                    // attributes
                    webfunnyRum.setGlobalAttribute(longKey("customerId"), 123456L);
                });
        binding.httpMeBad.setOnClickListener(
                v -> {
                    Span workflow = webfunnyRum.startWorkflow("Workflow With Error");
                    makeCall("https://asdlfkjasd.asdfkjasdf.ifi", workflow);
                });
        binding.httpMeNotFound.setOnClickListener(
                v -> {
                    Span workflow = webfunnyRum.startWorkflow("Workflow with 404");
                    makeCall("https://frontend-us.splunko11y.com/foobarbaz", workflow);
                });

        binding.volleyRequest.setOnClickListener(
                v -> {
                    VolleyExample volleyExample = new VolleyExample(webfunnyRum);
                    volleyExample.doHttpRequest();
                });

        sessionId.postValue(webfunnyRum.getRumSessionId());
    ...
    private Call.Factory buildOkHttpClient(WebfunnyRum webfunnyRum) {
        ...
        try {
            ...
            return webfunnyRum.createRumOkHttpCallFactory(
                    builder.sslSocketFactory(sslSocketFactory, (X509TrustManager) trustAllCerts[0])
                            .build());
        } catch (NoSuchAlgorithmException | KeyManagementException e) {
            e.printStackTrace();
            return webfunnyRum.createRumOkHttpCallFactory(builder.build());
        }
    }
