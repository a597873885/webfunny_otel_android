...
/** Entrypoint for the Webfunny OpenTelemetry Instrumentation for Android. */
public class WebfunnyRum {
    ...
    static final String LOG_TAG = "WebfunnyRum";
    static final String RUM_TRACER_NAME = "WebfunnyRum";
    ...
    static final AttributeKey<String> RUM_VERSION_KEY = stringKey("webfunny.rum.version");

    @Nullable private static WebfunnyRum INSTANCE;
    ...
    WebfunnyRum(OpenTelemetryRum openTelemetryRum, GlobalAttributesSupplier globalAttributes) {
        this.openTelemetryRum = openTelemetryRum;
        this.globalAttributes = globalAttributes;
    }

    /** Creates a new {@link WebfunnyRumBuilder}, used to set up a {@link WebfunnyRum} instance. */
    public static WebfunnyRumBuilder builder() {
        return new WebfunnyRumBuilder();
    }

    // for testing purposes
    static WebfunnyRum initialize(WebfunnyRumBuilder builder, Application application) {
        if (INSTANCE != null) {
            Log.w(LOG_TAG, "Singleton WebfunnyRum instance has already been initialized.");
            return INSTANCE;
        }

        if (builder.isSubprocessInstrumentationDisabled() && builder.isSubprocess) {
            INSTANCE = WebfunnyRum.noop();
        } else {
            INSTANCE =
        ...
        if (builder.isDebugEnabled()) {
            Log.i(
                    LOG_TAG,
                    "Webfunny RUM monitoring initialized with session ID: "
    ...
    /** Returns {@code true} if the Webfunny RUM library has been successfully initialized. */
    public static boolean isInitialized() {
        return INSTANCE != null;
    }

    /** Get the singleton instance of this class. */
    public static WebfunnyRum getInstance() {
        if (INSTANCE == null) {
            Log.d(LOG_TAG, "WebfunnyRum not initialized. Returning no-op implementation");
            return NoOpWebfunnyRum.INSTANCE;
        }
        return INSTANCE;
    }

    /**
     * Initialize a no-op version of the WebfunnyRum API, including the instance of OpenTelemetry that
     * is available. This can be useful for testing, or configuring your app without RUM enabled,
     * but still using the APIs.
     *
     * @return A no-op instance of {@link WebfunnyRum}
     */
    public static WebfunnyRum noop() {
        return NoOpWebfunnyRum.INSTANCE;
    }
    ...
    /**
     * Get the Webfunny Session ID associated with this instance of the RUM instrumentation library.
    ...
    /**
     * This method will enable Webfunny Browser-based RUM to integrate with the current Android RUM
     * Session. It injects a javascript object named "WebfunnyRumNative" into your WebView which
    ...
    public void integrateWithBrowserRum(WebView webView) {
        webView.addJavascriptInterface(new NativeRumSessionId(this), "WebfunnyRumNative");
    }
